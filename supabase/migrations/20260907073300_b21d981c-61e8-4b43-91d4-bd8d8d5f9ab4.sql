-- Restrict the security-definer quota helper: it is only reachable through the
-- authenticated app user, and never by anon/public.
...
-- Make sure the function can never leak another user's data: it derives the
-- user from auth.uid() only and returns nothing for anonymous callers.
...
BEGIN
  ...
  IF t IS NULL THEN
    INSERT INTO public.subscriptions(user_id) VALUES (uid) ON CONFLICT DO NOTHING;
    t := 'free';
  END IF;

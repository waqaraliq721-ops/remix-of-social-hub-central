    ...
    <div className="page-shell mx-auto w-full max-w-[1440px] space-y-6 p-4 sm:space-y-7 sm:p-6 lg:p-8 xl:px-10">
      <header className="flex flex-col gap-5 border-b border-border/80 pb-6 md:flex-row md:items-end md:justify-between">
        <div className="min-w-0">
          <div className="mb-2.5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-primary sm:text-[11px]"><span className="h-1.5 w-1.5 rounded-full bg-primary" />Workspace overview</div>
        ...
        <div className="flex w-full items-center gap-2 sm:w-auto"><Button variant="outline" asChild className="flex-1 sm:flex-none"><Link to="/accounts"><PlugZap className="h-4 w-4" /> Connect channel</Link></Button><Button asChild className="flex-1 sm:flex-none"><Link to="/compose"><Plus className="h-4 w-4" /> Create post</Link></Button></div>
      </header>

      <section className="motion-stagger grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {stats.map(({ label, value, icon: Icon, note }) => (
          <Card key={label} className="premium-card overflow-hidden"><CardContent className="p-4 sm:p-5">
            <div className="flex items-center justify-between"><div className="flex h-9 w-9 items-center justify-center rounded-lg border border-primary/10 bg-primary/[0.07] text-primary"><Icon className="h-[17px] w-[17px]" /></div><Activity className="h-4 w-4 text-muted-foreground/20" /></div>
            <div className="mt-5 flex items-end justify-between gap-3"><div><p className="text-[25px] font-semibold leading-none tracking-[-0.04em] tabular-nums">{value}</p><p className="mt-2 text-[13px] font-semibold">{label}</p></div><span className="mb-0.5 max-w-[110px] text-right text-[10px] font-medium leading-4 text-muted-foreground">{note}</span></div>
          </CardContent></Card>
        ))}
      </section>

      <div className="grid gap-5 xl:grid-cols-[1.35fr_1fr]">
        <Card className="premium-card overflow-hidden"><CardContent className="p-0">
          <div className="flex items-center justify-between gap-4 border-b border-border/70 px-5 py-4 sm:px-6"><div className="min-w-0"><h2 className="text-[13px] font-semibold tracking-[-0.01em]">Connected accounts</h2><p className="mt-1 text-[11px] text-muted-foreground">Your audience footprint by channel.</p></div><Button variant="ghost" size="sm" asChild><Link to="/accounts">Manage <ArrowUpRight className="ml-0.5 h-3.5 w-3.5" /></Link></Button></div>
          {accounts.isLoading ? <p className="px-5 py-12 text-sm text-muted-foreground sm:px-6">Loading accounts…</p> : (accounts.data ?? []).length === 0 ? <div className="px-5 py-12 sm:px-6 sm:py-14"><div className="flex h-10 w-10 items-center justify-center rounded-lg border border-border bg-muted/45"><Users className="h-4 w-4 text-muted-foreground" /></div><p className="mt-4 text-sm font-semibold">No accounts connected</p><p className="mt-1 max-w-sm text-xs leading-5 text-muted-foreground">Connect a channel to start building your workspace and tracking its audience.</p><Button variant="outline" size="sm" asChild className="mt-4"><Link to="/accounts">Connect an account <ChevronRight className="ml-0.5 h-3.5 w-3.5" /></Link></Button></div> : <div className="divide-y divide-border/70">{(accounts.data ?? []).map((a) => <div key={a.id} className="premium-row flex min-w-0 items-center gap-3 px-5 py-3.5 sm:px-6"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-border bg-muted/25"><PlatformIcon platform={a.platform} className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{a.display_name ?? a.handle}</p><p className="truncate text-[11px] text-muted-foreground">{a.handle}</p></div><div className="shrink-0 text-right"><p className="text-sm font-semibold tabular-nums">{a.followers.toLocaleString()}</p><p className="text-[10px] text-muted-foreground">followers</p></div></div>)}</div>}
        </CardContent></Card>

        <Card className="premium-card overflow-hidden"><CardContent className="p-0">
          <div className="flex items-center justify-between gap-4 border-b border-border/70 px-5 py-4 sm:px-6"><div className="min-w-0"><h2 className="text-[13px] font-semibold tracking-[-0.01em]">Recent posts</h2><p className="mt-1 text-[11px] text-muted-foreground">Latest publishing activity.</p></div><Button variant="ghost" size="sm" asChild><Link to="/calendar">Calendar <ArrowUpRight className="ml-0.5 h-3.5 w-3.5" /></Link></Button></div>
      ...
      <Card className="premium-card overflow-hidden"><CardContent className="p-0">
        <div className="flex items-center justify-between gap-3 px-5 py-4 sm:px-6"><div><h2 className="text-[13px] font-semibold tracking-[-0.01em]">Channel coverage</h2><p className="mt-1 text-[11px] text-muted-foreground">Platforms available in your workspace.</p></div><span className="shrink-0 rounded-full border border-border bg-muted/35 px-2.5 py-1 text-[10px] font-medium text-muted-foreground">{PLATFORMS.length} channels</span></div>
